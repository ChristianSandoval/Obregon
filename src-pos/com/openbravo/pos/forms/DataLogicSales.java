...
import com.openbravo.pos.inventory.CapturasInventarioInfo;
...
public CustomerInfoExt findCustomerExtById(String id)
    ...
    public final SentenceList getCapturasList() {
        return new StaticSentence(s
            , "SELECT UUID AS ID, DATENEW AS NAME,CASE WHEN REASON='1' THEN '+compra' WHEN REASON='2' THEN '+devolucion' WHEN REASON='3' THEN '+traspaso' WHEN REASON='4' THEN '+traspaso' END AS RAZON, L.NAME FROM STOCKDIARY S INNER JOIN LOCATIONS L ON S.LOCATION=L.ID WHERE UUID IS NOT NULL AND REASON>0 GROUP BY UUID, DATENEW,REASON,L.NAME ORDER BY DATENEW DESC LIMIT 0,20"
            , null
            , new SerializerRead() { public Object readValues(DataRead dr) throws BasicException {
                return new TaxCustCategoryInfo(dr.getString(1),com.openbravo.format.Formats.TIMESTAMP.formatValue(dr.getTimestamp(2))+" ("+dr.getString(3)+" "+dr.getString(4)+")");
            }});
    }
        ...
        Transaction t = new Transaction(s) {
            public Object transact() throws BasicException {
                ...
                for (final PaymentInfo p : ticket.getPayments()) {
                    ...
                    if ("debt".equals(p.getName()) || "debtpaid".equals(p.getName())|| "debtpaiddesc".equals(p.getName())) {
